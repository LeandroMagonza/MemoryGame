...
public class PersistanceManager : MonoBehaviour
{
    ...
    public bool saveOnNextFrame = false;
    ...
    public void SaveUserData()
    {
        ...
        // File.WriteAllText(filePath, json);
        // StartCoroutine(FirebaseManager.Instance.SaveUserData(JsonToDictionary(json)));
        StartCoroutine(FirebaseManager.Instance.SaveUserData(json));
    ...
    private static Dictionary<string, object> ParseJToken(JToken token)
    {
        ...
        foreach (var item in token)
        {
            ...
            if (jProperty != null)
            {
                ...
                {
                    result.Add(jProperty.Name, ParseJToken(jProperty.Value));
                }
                else if (jProperty.Value is JArray)
                {
                    result.Add(jProperty.Name, ParseJArray(jProperty.Value as JArray));
                }
                else
                {
                    result.Add(jProperty.Name, jProperty.Value);
                }
            }
        }
        return result;
    ...
    private static List<object> ParseJArray(JArray array)
    {
        ...
        foreach (var item in array)
        {
            ...
            {
                list.Add(ParseJArray(item as JArray));
            }
            else
            {
                list.Add(item);
